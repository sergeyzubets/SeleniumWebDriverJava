...
public class LoginTest extends BaseTest {
    ...
    public void loginTest(String username, String password) throws InterruptedException {
        log.info("loginTest");
        getDriver().findElement(LOGIN_BUTTON_ON_MAIN_PAGE).click();

        try {
            WebElement currentAccount = getDriver().findElement(CURRENT_ACCOUNT_VALUE);

            if (currentAccount.isDisplayed()) {
                currentAccount.click();
                getDriver().findElement(LOGIN_TO_ANOTHER_ACCOUNT_BUTTON).click();
            }
        ...
        WebElement loginField = getDriver().findElement(LOGIN_FIELD);
        loginField.sendKeys(username);
        getDriver().findElement(LOGIN_BUTTON_ON_LOGIN_PAGE).click();
        ...
        WebElement accountName = getDriver().findElement(ACCOUNT_NAME);
        Assert.assertEquals(username, accountName.getText(), LOGIN_IS_NOT_SUCCESSFUL);

        log.info("Log out");
        accountName.click();
        WebElement logout = getDriver().findElement(LOGOUT_BUTTON);
        logout.click();
    }
