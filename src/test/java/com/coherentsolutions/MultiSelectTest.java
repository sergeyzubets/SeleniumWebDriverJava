...
public class MultiSelectTest extends BaseTest {
    ...
    public void multiSelectListTest() {
        List<Integer> optionPositions = getRandomOptionPositions(getDriver());
        selectOptionsInMultiSelectList(getDriver(), optionPositions);

        Assert.assertEquals(
                getActualResult(getDriver()),
                getExpectedResult(getDriver(), optionPositions),
                MULTISELECT_FAIL);
    }
}